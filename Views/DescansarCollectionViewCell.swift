...
class DescansarCollectionViewCell: UICollectionViewCell {
    ...
    static let identifier = "descansarCell"
    
    let imageView: UIImageView = {
        let imageView = UIImageView()
        imageView.image = UIImage(named: "Rectangle 361")
...
@available(iOS 13, *)
struct DescansarCollectionViewCell_Preview: PreviewProvider {
    static var previews: some View {
        // view controller using programmatic UI
        Group {
            DescansarCollectionViewCell().showPreview().previewLayout(.fixed(width: 157.0, height: 136.0))
