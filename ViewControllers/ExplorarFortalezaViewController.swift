//
//  ExplorarFortalezaViewController.swift
//  CarapacaApp
//
...
class ExplorarFortalezaViewController: UIViewController {

    let exploreFortalezaView = ExploreFortalezaView()
...
//        navigationItem.title = "Nome do Roteiro"
    ...
    func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            exploreFortalezaView.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            exploreFortalezaView.trailingAnchor.constraint(equalTo: view.trailingAnchor)
        ])
        ...
        NSLayoutConstraint.activate([
            scrollView.topAnchor.constraint(equalTo: exploreFortalezaView.bottomAnchor),
...
extension ExplorarFortalezaViewController: UICollectionViewDelegate, UICollectionViewDataSource {
...
//        cell?.configure(imagem: UIImage(named: "Rectangle 361") ?? UIImage(), lugar: "Lugar")
        cell?.setup(with: fortalezaPessoas[indexPath.row])
...
//        cell?.configure(imagem: UIImage(named: "Rectangle 361") ?? UIImage(), lugar: "Renda")
        cell?.setup(with: fortalezaRenda[indexPath.row])
        return cell ?? UICollectionViewCell()
...
//        cell?.configure(imagem: UIImage(named: "Rectangle 361") ?? UIImage(), lugar: "Descansar")
        cell?.setup(with: fortalezaDescansar[indexPath.row])
        return cell ?? UICollectionViewCell()
...
@available(iOS 13, *)
struct ExplorarFortalezaViewController_Preview: PreviewProvider {
    static var previews: some View {
        // view controller using programmatic UI
        Group {
            ExplorarFortalezaViewController().showPreview().previewDevice("iPhone SE (3rd generation)")
